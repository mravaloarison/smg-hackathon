  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mb-4 flex max-h-60 flex-col gap-1 overflow-y-auto">
          {playlists.length === 0 && (
          ...
          {playlists.map((playlist) => {
            ...
            return (
              <button
                ...
                disabled={alreadyIn}
                className="flex items-center justify-between rounded-lg px-3 py-2 text-left text-sm text-neutral-900 transition hover:bg-neutral-100 disabled:opacity-50 dark:text-neutral-100 dark:hover:bg-neutral-800"
              >
                <span>{playlist.name}</span>
                {alreadyIn && <span className="text-xs text-neutral-400">Added</span>}
              </button>
            );
          })}
        </div>
