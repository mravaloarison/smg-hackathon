...
export default function PlaylistCard({
  ...
  const hasActions = onRename || onDelete || onInvite;
  ...
  const pillBase =
    "rounded-full border border-neutral-300 px-3 py-1 text-xs font-medium text-neutral-700 transition hover:bg-neutral-100 dark:border-neutral-700 dark:text-neutral-200 dark:hover:bg-neutral-800";
  const pillRed =
    "rounded-full border border-red-200 px-3 py-1 text-xs font-medium text-red-600 transition hover:bg-red-50 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-950";
  ...
  return (
    <div className="relative flex items-center gap-3 rounded-xl p-3 transition hover:bg-neutral-100 dark:hover:bg-neutral-800">
      ...
      {hasActions && (
        <>
          {/* Desktop: stacked pill buttons */}
          <div className="hidden flex-shrink-0 flex-col gap-1.5 md:flex">
            {onRename && (
              <button
                type="button"
                onClick={(e) => { e.preventDefault(); onRename(playlist); }}
                className={pillBase}
              >
                Rename
              </button>
            ...
            {onInvite && (
              <button
                type="button"
                onClick={(e) => { e.preventDefault(); onInvite(playlist); }}
                className={pillBase}
              >
                Invite a friend
              </button>
